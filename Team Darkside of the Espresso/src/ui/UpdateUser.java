...
public class UpdateUser extends JPanel {
	...
	private void initialize() {
		...
		txtBirthDate.setText(user.getUserInformation().getBirthDate()
				.toString());
	...
	private void createUser() {
		...
		try {
			...
			final User currentUser = Main.getCurrentUser();
			final String oldName = currentUser.getUserInformation().getName();

			currentUser.setUsername(txtUsername.getText());
			currentUser.setPasswordHash(Crypto.getSha1Hash(strPassword
					.toString()));
			currentUser.setGender(rdoMale.isSelected() ? GenderType.MALE
					: GenderType.FEMALE);

			final UserInfo currentUserInfo = currentUser.getUserInformation();
			...
			if (db.canUpdateUser(
				...
				if (!SystemLog.canLogMessage("User " + userInfo.getName()
					...
					if (!SystemLog.canLogMessage(
						...
						System.out
								.println("Error: Could not log message \"Error: User "
										+ userInfo.getName()
